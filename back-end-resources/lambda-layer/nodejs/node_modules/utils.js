...
// Retrieve meeting from the meeting table by meeting title
exports.getBreakoutMeeting = async (meetingTitle) => {
  var params = {
      KeyConditionExpression: 'ParentMeeting = :parent_meeting',
      ExpressionAttributeValues: {
          ':parent_meeting': {'S': meetingTitle}
      },
      TableName: meetingsTableName,
  ...
  for (var i = 0; i < len; i++) {
    console.log("i: ",i);
    console.log("Title: ", result.Items[i].Title.S)
    console.log("Data: ", result.Items[i].Data.S)
    breakoutRooms.push({ Title: result.Items[i].Title.S, Data: result.Items[i].Data.S } );
  }
  console.log(breakoutRooms);
  return breakoutRooms;
};

// Creates Breakout Meeting Information
exports.putBreakoutMeeting = async (meeting, breakout, meetingInfo) => {
  await ddb.putItem({
    TableName: meetingsTableName,
    Item: {
      'Title': { S: breakout },
      'Data': { S: JSON.stringify(meetingInfo) },
      'ParentMeeting': { S: meeting },
      'TTL': {
        N: '' + oneDayFromNow
      }
    }
